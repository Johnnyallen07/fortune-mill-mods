...
- Dart 靶区属性会做安全限制：`DARTS_BULLSEYE_SIZE <= 1x`、`DARTS_BULLSEYE_COUNT <= 20`，避免高等级靶心区域在点击/判定时进入无效状态。
...
脚本会先备份 `save_game.sav`，然后只把 `DARTS_BULLSEYE_SIZE` 对应升级降到 `0`、`DARTS_BULLSEYE_COUNT` 对应升级降到 `20`。
