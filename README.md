...
如果 `.app` 内部 runtimeconfig 被 macOS 权限保护，直接用这个脚本启动游戏：

```bash
./launch-fortune-mill-modded.sh
```

脚本会设置 `DOTNET_STARTUP_HOOKS` 指向游戏根目录的 `BepInEx.NET.CoreCLR.dll`，再启动：

```text
../Fortune Mill.app/Contents/MacOS/Fortune Mill
```
